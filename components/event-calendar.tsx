...
import { X, ChevronLeft, ChevronRight, CalendarDays, Lock, LockOpen } from "lucide-react"
...
function LockToggleButton({ isLocked, onToggle, size = "sm" }: { isLocked: boolean; onToggle: () => void; size?: "sm" | "md" }) {
...
function SplitSleepToggle({ checked, onToggle, size = "sm" }: { checked: boolean; onToggle: () => void; size?: "sm" | "md" }) {
  const isMd = size === "md"
  return (
    <button
      onClick={(e) => { e.stopPropagation(); onToggle() }}
...
function DayMemoTextarea({ value, onChange, className }: { value: string; onChange: (v: string) => void; className?: string }) {
  return (
    <textarea
      value={value}
